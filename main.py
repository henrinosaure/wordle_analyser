...
from optimal_finder import best_word_finder, determine_strategy

for i in range(6):
    attempt1 = input(f"Attempt#{i+1}: ").upper()
    colors1= input("What are the colors for the attempt (example: gybbb) : ").upper()
    print("--------------------------------------------------------")
    if colors1=="GGGGG":
    ...
    if len(remaining_words)>3:
        print("Loading best word...")
    best_word = best_word_finder(remaining_words, loading=True, update_interval=1)
    if best_word=={"AAHED":0.0} or best_word=={"ABACA":0.0}:
        print(f"The best word to play is {remaining_words[0]}")
    else:
        print(f"the best word to play is {best_word}")
    print("--------------------------------------------------------")
